...
import com.xzfg.app.util.SMSUtil.ContactIdentification;

public class SmsMmsMessage {
    private static final String TAG = SmsMmsMessage.class.getName();
    ...
    public SmsMmsMessage(Context _context, SmsMessage[] messages, long _timestamp) {
        ...
        try {
            ...
            if (BuildConfig.DEBUG) Log.v(TAG, "SmsMmsMessage<init> exception: " + e.toString());
        }
        messageBody = body;
        ...
        // address
        if (fromEmailGateway) {
            if (BuildConfig.DEBUG) Log.v(TAG, "Sms came from email gateway");
            contactIdentify = SMSUtil.getPersonIdFromEmail(context, fromAddress);
            contactName = fromAddress;
        } else { // Else lookup contactId by phone number
            if (BuildConfig.DEBUG) Log.v(TAG, "Sms did NOT come from email gateway");
            contactIdentify = SMSUtil.getPersonIdFromPhoneNumber(context, fromAddress);
    ...
    public SmsMmsMessage(Context _context, long _messageId, long _threadId, long _timestamp,
        ...
        fromAddress = SMSUtil.getMmsAddress(context, messageId);
        ...
        ContactIdentification contactIdentify =
                SMSUtil.getPersonIdFromPhoneNumber(context, fromAddress);
    ...
    public SmsMmsMessage(Context _context, String _fromAddress, String _messageBody,
        ...
        if (SMSUtil.isEmailAddress(fromAddress)) {
            contactIdentify = SMSUtil.getPersonIdFromEmail(context, fromAddress);
            fromEmailGateway = true;
        } else {
            contactIdentify = SMSUtil.getPersonIdFromPhoneNumber(context, fromAddress);
    ...
    public CharSequence getFormattedTimestamp() {
        ...
        try {
            ...
            if (BuildConfig.DEBUG) Log.v(TAG, "Error formatting timestamp + " + timestamp);
            formattedTime = "";
        }
    ...
    public boolean shouldNotify() {
        if (BuildConfig.DEBUG) Log.v(TAG, "shouldNotify() - notify is " + notify);
        return notify;
    }
    ...
    public void locateThreadId() {
        if (threadId == 0) {
            threadId = SMSUtil.findThreadIdFromAddress(context, fromAddress);
        }
    }
    ...
    public void locateMessageId() {
        if (messageId == 0) {
            ...
            messageId = SMSUtil.findMessageId(
