...
public class SMSDeliveryTimerService extends IntentService {
    private static final String TAG = SMSDeliveryTimerService.class.getName();
    ...
    public SMSDeliveryTimerService() {
        super(TAG);
    }
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if (sFilePath.length()>0) {
            logFile = new File(SMSUtil.getFilePath(application), SMSUtil.LOG_FILE_NAME_SMS);
        }
        ...
        // schedule task
        mTimer.scheduleAtFixedRate(new SMSDeliveryTimerTask(), 0, deliveryInterval);
    }

    class SMSDeliveryTimerTask extends TimerTask {
